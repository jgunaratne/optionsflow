...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] flex-col">
      ...
      <div className="mb-4 flex items-center justify-between border-b border-white/10 pb-4 mt-2">
        ...
        {messages.length > 0 && (
          <button onClick={() => { clearHistory(); }}
            className="flex items-center gap-2 border border-white/10 bg-white/5 px-4 py-2 text-sm font-bold text-zinc-400 transition-all hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 rounded-2xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 max-w-xl w-full">
              {STARTER_PROMPTS.map((prompt, i) => (
                <button key={i} onClick={() => { setInput(prompt); }}
                  className="group flex items-center justify-between border border-white/10 bg-white/5 px-5 py-4 text-left text-sm font-bold text-zinc-400 transition-all hover:border-primary/50 hover:bg-primary/5 hover:text-zinc-200 rounded-2xl">
                  <span>{prompt}</span>
                  <div className="h-5 w-5 rounded-2xl bg-zinc-900 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      {/* Input Section */}
      <div className="mt-6 flex gap-5 max-w-4xl mx-auto w-full">
        <div className="relative flex-1 group">
          ...
          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
             <kbd className="hidden sm:inline-flex h-5 items-center gap-1 rounded-2xl border border-white/10 bg-white/5 px-1.5 font-mono text-[10px] font-medium text-zinc-400 opacity-100">
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 bg-gradient-to-r from-zinc-600 to-zinc-800 px-8 py-4 text-sm font-bold text-white shadow-lg shadow-black/40 hover:shadow-black/60 hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:grayscale rounded-2xl transition-all"
        >
