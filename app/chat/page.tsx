...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] flex-col">
      ...
      <div className="mb-4 flex items-center justify-between border-b border-white/5 pb-4 mt-2">
        ...
        {messages.length > 0 && (
          <button onClick={() => { clearHistory(); }}
            className="flex items-center gap-2 border border-white/10 bg-white/5 px-4 py-2 text-xs font-bold text-zinc-400 transition-all hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 rounded-md">
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto border border-white/5 bg-zinc-950/50 backdrop-blur-sm p-6 space-y-6 rounded-md shadow-inner">
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center gap-8">
            <div className="relative">
              <div className="absolute inset-0 bg-primary/20 blur-2xl rounded-full" />
              <div className="relative flex h-16 w-16 items-center justify-center border border-primary/30 bg-zinc-900 text-primary rounded-md shadow-2xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl w-full">
              {STARTER_PROMPTS.map((prompt, i) => (
                <button key={i} onClick={() => { setInput(prompt); }}
                  className="group flex items-center justify-between border border-white/5 bg-white/5 px-5 py-4 text-left text-xs font-bold text-zinc-400 transition-all hover:border-primary/50 hover:bg-primary/5 hover:text-zinc-200 rounded-md">
                  <span>{prompt}</span>
          ...
          <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full">
            {messages.map((msg, i) => (
              <div key={i} className={cn("flex gap-4", msg.role === 'user' ? "flex-row-reverse" : "flex-row")}>
                <div className={cn(
                  "flex h-10 w-10 shrink-0 items-center justify-center border text-xs shadow-lg rounded-md",
                ...
                <div className={cn(
                  "max-w-[80%] border px-5 py-4 text-sm leading-relaxed shadow-sm rounded-md",
                  msg.role === 'user'
            ...
            {loading && (
              <div className="flex gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center border border-primary/30 bg-zinc-900 text-primary rounded-md animate-pulse">
                  <Loader2 className="h-5 w-5 animate-spin" />
                </div>
                <div className="bg-white/5 border border-white/5 px-5 py-4 rounded-md rounded-tl-none flex items-center gap-3">
      ...
      <div className="mt-6 flex gap-3 max-w-4xl mx-auto w-full">
        <div className="relative flex-1 group">
          <input
            ...
            className="w-full border border-white/10 bg-zinc-900/50 px-6 py-4 text-sm text-white placeholder-zinc-600 outline-none focus:border-primary/50 focus:bg-zinc-900 transition-all rounded-md shadow-xl"
            disabled={loading}
          />
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 bg-gradient-to-r from-primary to-indigo-600 px-8 py-4 text-sm font-bold text-white shadow-lg shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-30 disabled:grayscale rounded-md transition-all"
        >
