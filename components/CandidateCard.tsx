...
export default function CandidateCard({ candidate, onAddToQueue, inQueue, disabled }: CandidateCardProps) {
  ...
  return (
    <div className={cn(
      "group relative flex flex-col justify-between overflow-hidden rounded-2xl border bg-zinc-900/30 p-6 backdrop-blur-md shadow-xl transition-all duration-300 focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2 focus-within:ring-offset-black",
      isRejected ? "opacity-75" : "hover:bg-zinc-800/80 hover:shadow-xl hover:shadow-black/60 hover:-translate-y-1",
      flagBorderClass
    )}>
      {/* Background Glow */}
      <div className={cn(
        "absolute -right-8 -top-8 h-24 w-24 rounded-2xl-full blur-3xl opacity-20 transition-opacity group-hover:opacity-40",
      ...
      <div>
        ...
        <div className="mb-5 flex items-start justify-between">
          <div>
            ...
            <div className="mt-1 flex items-center gap-5">
              <span className="rounded-2xl bg-white/10 border border-white/20 px-2 py-0.5 text-sm font-bold text-zinc-300">
                {candidate.strategy}
              </span>
              <span className="text-sm font-semibold text-zinc-400">${candidate.underlying_price.toFixed(2)}</span>
            </div>
          </div>
          <div className={cn("flex items-center gap-2 rounded-2xl px-3 py-1 backdrop-blur-md border", flagBgClass, flagBorderClass)}>
            <div className={cn("h-2 w-2 rounded-2xl-full", isRejected ? 'bg-zinc-400' : isGreen ? 'bg-emerald-400' : isYellow ? 'bg-amber-400' : 'bg-red-400')} aria-hidden="true" />
            <span className={cn("text-sm font-bold uppercase tracking-wider", flagColorClass)}>
              {recommendationLabel}
            </span>
          </div>
        </div>

        <div className={cn("mb-5 rounded-2xl border px-3 py-2.5", flagBgClass, flagBorderClass)}>
        ...
        {/* Contract Details */}
        <div className="mb-5 flex rounded-2xl bg-zinc-950/30 p-1 border border-white/10">
        ...
        <div className="mb-5 grid grid-cols-2 gap-5">
          <div className="rounded-2xl bg-zinc-950/30 p-3 border border-white/10">
            <div className="text-sm text-zinc-400 uppercase font-bold tracking-tight">Money In</div>
            <div className="text-lg font-extrabold text-emerald-400">${candidate.premium.toFixed(2)}</div>
          </div>
          <div className="rounded-2xl bg-zinc-950/30 p-3 border border-white/10">
            <div className="text-sm text-zinc-400 uppercase font-bold tracking-tight">Worst Case</div>
            <div className="text-lg font-extrabold text-red-400">${candidate.max_loss.toLocaleString()}</div>
          </div>
          <div className="rounded-2xl bg-zinc-950/30 p-3 border border-white/10">
            <div className="text-sm text-zinc-400 uppercase font-bold tracking-tight">Chance It Works</div>
            <div className="text-lg font-extrabold text-zinc-100">{(candidate.pop * 100).toFixed(0)}%</div>
          </div>
          <div className="rounded-2xl bg-zinc-950/30 p-3 border border-white/10">
            <div className="text-sm text-zinc-400 uppercase font-bold tracking-tight">Volatility</div>
            <div className="text-lg font-extrabold text-zinc-100">{candidate.iv_rank.toFixed(1)}</div>
          </div>
        </div>

        {/* Greeks */}
        <div className="mb-5 flex justify-between px-2 text-sm font-semibold text-zinc-400" aria-label="Greeks">
          <span className="flex items-center gap-1">Direction Risk <span className="text-zinc-200">{candidate.delta.toFixed(3)}</span></span>
          <span className="flex items-center gap-1">Time Decay <span className="text-zinc-200">{candidate.theta.toFixed(4)}</span></span>
          <span className="flex items-center gap-1">Hype Risk <span className="text-zinc-200">{candidate.vega.toFixed(3)}</span></span>
        </div>

        {/* AI Analysis */}
        <div className="mb-6 rounded-2xl border border-white/10 bg-zinc-950/30 p-6 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-zinc-500 to-zinc-700" aria-hidden="true" />
          <div className="mb-2 flex items-center justify-between">
            <span className="text-sm font-bold uppercase tracking-wider text-zinc-400">AI Insight</span>
            <span className={cn(
              "text-sm font-black",
          ...
          {isRejected && candidate.rejection_reason && (
            <p className="mt-2 text-sm font-medium text-zinc-500">{candidate.rejection_reason}</p>
          )}
        </div>
      </div>
      ...
      <button
        ...
        className={cn(
          "relative flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-bold transition-all duration-300 overflow-hidden focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-black",
          inQueue
