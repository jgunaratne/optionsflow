  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        disabled={switching}
        className={cn(
          "flex items-center gap-1.5 border px-2 py-0.5 text-[10px] font-black transition-all",
          meta.bg,
          switching ? 'opacity-50 cursor-not-allowed' : 'hover:bg-zinc-800'
        )}
      >
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 w-32 border border-zinc-800 bg-black p-1 shadow-2xl">
          {supported.map(broker => {
            const m = BROKER_META[broker] || { label: broker.toUpperCase(), color: 'text-zinc-400', bg: '' };
            ...
            return (
              <button
                ...
                disabled={switching}
                className={cn(
                  "flex w-full items-center justify-between px-2 py-1.5 text-left text-[10px] font-bold transition-colors",
      ...
      <div className="flex h-10 w-full items-center justify-between px-4">
        ...
        <div className="flex items-center gap-4">
          <Link href="/" className="flex items-center gap-2">
            <div className="flex h-6 w-6 items-center justify-center bg-primary text-[10px] font-black text-black">
              OF
            </div>
            ...
            {links.map(link => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  className={cn(
                    "px-3 py-1 text-[11px] font-black transition-colors border-r border-zinc-900",
                    isActive
        ...
        <div className="flex items-center gap-4 h-full">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-bold">
            <span className="text-zinc-700">MKT:</span>
            <span className={cn("uppercase", statusColor)}>
              {marketStatus.status.replace('_', ' ')}
            </span>
            <div className={cn("h-1 w-1 rounded-full", connected ? "bg-emerald-500" : "bg-red-500")} />
          </div>

          {/* VIX */}
          <div className="flex items-center gap-1 text-[10px] font-bold">
            <span className="text-zinc-700">VIX:</span>
